...
//  ------------------------------------------------------------------------------------------
	...
	auto & operator*=(Integer other)
	{
		...
		for (auto i = std::size_t { 0 }; i < m_size; ++i)
		{
			...
			for (auto j = std::size_t { 0 }; (j < other.m_size) || remainder; ++j)
			{
...
//  ------------------------------------------------------------------------------------------
	...
	friend auto operator==(Integer const & lhs, Integer const & rhs) -> bool
	{
		...
		for (auto i = std::size_t { 0 }; i < lhs.m_size; ++i)
		{
...
//  ------------------------------------------------------------------------------------------
	...
	friend auto & operator<<(std::ostream & stream, Integer const & integer)
	{
		...
		{
			stream << std::setw(static_cast < int > (Integer::s_step))
			       << std::setfill('0')
			       << integer.m_digits[i];
		}
...
//  ------------------------------------------------------------------------------------------
	...
	friend auto multiply(Integer const & x, Integer const & y) -> Integer
	{
		if (auto size = std::max(x.m_size, y.m_size); size > 1)
		{
			...
			for (auto i = std::size_t { 0 }; i < step; ++i) { x1.m_digits[i       ] = x.m_digits[i]; }
			...
			for (auto i = std::size_t { 0 }; i < step; ++i) { y1.m_digits[i       ] = y.m_digits[i]; }
			...
			for (auto i = std::size_t { 1 }; i < step; ++i)
			{
				base *= Integer::s_base;
			}
...
private :
	...
	void parse(std::string const & string)
	{
		...
		for (auto i = static_cast < long long > (string.size()) - 1; i >= 0; i -= s_step)
		{
...
//  ------------------------------------------------------------------------------------------
	...
	static inline auto s_size = static_cast < std::size_t > (1'000);
